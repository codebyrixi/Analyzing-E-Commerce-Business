--1. Menampilkan rata-rata jumlah customer aktif bulanan
--   (MAU / Monthly Active User) untuk setiap tahun
SELECT year, FLOOR(AVG(cust_total)) AS avg_mau
FROM (
  SELECT 
  	date_part('year', od.order_purchase_timestamp) AS year,
  	date_part('month', od.order_purchase_timestamp) AS month,
  	COUNT(DISTINCT cd.customer_unique_id) AS cust_total
...
SELECT year, count(customer_unique_id) AS total_cust_repeat
FROM (
  SELECT
  	date_part('year', od.order_purchase_timestamp) AS year,
  	cd.customer_unique_id,
...
cte_new_cust AS (
  SELECT year, COUNT(customer_unique_id) AS total_new_cust
  FROM (
  	SELECT
  		Min(date_part('year', od.order_purchase_timestamp)) AS year,
  		cd.customer_unique_id
